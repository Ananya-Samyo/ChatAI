    ...
    <div class="flex flex-col h-screen bg-slate-50">
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-4" id="chat-window">
            <div v-for="(msg, index) in chatHistory" :key="index"
                ...
                <div
                    ...
                    <p v-else v-html="formatMessage(msg.text)" class="leading-relaxed"></p>
        ...
        <footer class="p-4 bg-white border-t border-slate-100">
            <div class="flex gap-2">
                <input v-model="userInput" @keyup.enter="sendMessage" placeholder="พิมพ์อะไรบางอย่าง..."
            ...
            <p class="text-[10px] text-slate-400 text-center mt-2 italic">
                ⚠️ คำเตือน: เนื้อหาและการตอบโต้ทั้งหมดเป็นบทบาทสมมุติ โปรดใช้วิจารณญาณ
            </p>
        </footer>
    </div>
</template>
...
const formatMessage = (text) => {
    if (!text) return '';
    return text.replace(/\*(.*?)\*/g, '<span class="text-gray-400 font-light italic">($1)</span>');
};
